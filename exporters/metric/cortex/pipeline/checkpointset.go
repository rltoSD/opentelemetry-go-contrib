...
func (p *CheckpointSet) Add(timestamp time.Time, desc *metric.Descriptor, newAgg export.Aggregator, labels ...label.KeyValue) (agg export.Aggregator, added bool) {
	...
	rec := export.NewRecord(desc, &elabels, p.resource, newAgg.Aggregation(), timestamp, timestamp)
